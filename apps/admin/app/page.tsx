...
function PlatformSettings({data}:{data:any}) {
 ...
 async function submit(e:React.FormEvent<HTMLFormElement>){e.preventDefault();setSaving(true);setMessage("");const f=new FormData(e.currentTarget);const body={platformName:f.get("platformName"),supportEmail:f.get("supportEmail"),defaultPlan:f.get("defaultPlan"),defaultSeats:Number(f.get("defaultSeats")),allowTrials:f.get("allowTrials")==="on",trialDays:Number(f.get("trialDays")),groqModel:f.get("groqModel")??data?.groqModel??"llama-3.3-70b-versatile",groqApiKey:f.get("groqApiKey")??"",leadSearchProvider:f.get("leadSearchProvider")??data?.leadSearchProvider??"SERPER",leadSearchApiKey:f.get("leadSearchApiKey")??""};const r=await fetch(`${apiUrl}/admin/settings`,{method:"PUT",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});setMessage(r.ok?"Settings saved":"Could not save settings");setSaving(false);}
 return <div className="platform-settings-layout"><Card><CardHeader><CardTitle>General</CardTitle><CardDescription>Defaults for the whole Hyaw CRM platform.</CardDescription></CardHeader><CardContent><form className="business-form" onSubmit={submit}><div className="form-grid"><label>Platform name<input name="platformName" defaultValue={data?.platformName??"Hyaw CRM"} required/></label><label>Support email<input name="supportEmail" type="email" defaultValue={data?.supportEmail??"support@hyaw.tech"} required/></label><label>Default plan<input name="defaultPlan" defaultValue={data?.defaultPlan??"Starter"} required/></label><label>Default seats<input name="defaultSeats" type="number" min="1" defaultValue={data?.defaultSeats??5} required/></label><label>Trial days<input name="trialDays" type="number" min="0" defaultValue={data?.trialDays??14} required/></label></div><label className="toggle-row"><span><strong>Allow trials</strong><small>Allow newly created businesses to start on a trial.</small></span><input name="allowTrials" type="checkbox" defaultChecked={data?.allowTrials??true}/></label>{message?<p className="settings-message">{message}</p>:null}<div className="business-form-actions"><Button disabled={saving} type="submit">{saving?"Saving...":"Save platform settings"}</Button></div></form></CardContent></Card><Card><CardHeader><CardTitle>AI & lead discovery</CardTitle><CardDescription>Super Admin-only credentials used by lead search and the Groq calling expert.</CardDescription></CardHeader><CardContent><div className="business-form"><label>Groq model<input name="groqModel" form="ai-settings-form" defaultValue={data?.groqModel??"llama-3.3-70b-versatile"} required/></label><label>Groq API key<input name="groqApiKey" form="ai-settings-form" type="password" autoComplete="new-password" placeholder={data?.groqApiKeyMasked?"Configured: "+data.groqApiKeyMasked:"Paste Groq API key"}/></label><label>Lead search provider<select name="leadSearchProvider" form="ai-settings-form" defaultValue={data?.leadSearchProvider??"SERPER"}><option value="SERPER">Serper</option><option value="BRAVE">Brave Search</option><option value="TAVILY">Tavily</option><option value="EXA">Exa</option></select></label><label>Search API key<input name="leadSearchApiKey" form="ai-settings-form" type="password" autoComplete="new-password" placeholder={data?.leadSearchApiKeyMasked?"Configured: "+data.leadSearchApiKeyMasked:"Paste selected provider API key"}/></label><p>Keys are encrypted before storage and never returned in full.</p><form id="ai-settings-form" onSubmit={submit}><Button disabled={saving} type="submit">{saving?"Saving...":"Save AI settings"}</Button></form></div></CardContent></Card></div>;
}
