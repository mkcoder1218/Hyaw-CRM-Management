...
export function formatSearchCandidate(candidate:SearchCandidate,campaign:{industry?:string|null;location?:string|null}):FormattedLead|null{
 ...
 const url=new URL(candidate.link),website=directoryHost.test(url.hostname)?undefined:url.origin;
 return {name:company,company,website,location:campaign.location||undefined,industry:campaign.industry||undefined,sourceUrl:candidate.link,sourceName:candidate.source||url.hostname,description:candidate.snippet?strip(candidate.snippet).slice(0,1200):undefined,rawData:candidate};
}
