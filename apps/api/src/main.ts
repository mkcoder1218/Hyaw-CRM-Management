...
app.use("/api/health",healthRouter);app.use("/api/leads",leadsRouter);app.use("/api/sops",sopsRouter);
app.use((req,res)=>res.status(404).json({message:"Route not found",path:req.path}));
app.listen(port,()=>console.log("Hyaw CRM API listening on http://localhost:"+port));
