...
workspaceRouter.get("/requests",requirePermission("lead.view"),async(req,res,next)=>{try{res.json({data:await prisma.clientRequest.findMany({where:{tenantId:tenantId(req)},include:{lead:{select:{id:true,firstName:true,lastName:true,company:true,phone:true,email:true,hrCallOutcome:true}},submittedBy:{select:{firstName:true,lastName:true}}},orderBy:{createdAt:"desc"}})})}catch(e){next(e)}});
